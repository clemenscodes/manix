...
fn main() -> Result<()> {
    ...
    if let Some(generator) = opt.generator {
        ...
        if opt.man {
            ...
            let cache_dir = xdg::BaseDirectories::with_prefix("manix")
                .context("Failed to get a cache directory")?;
            ...
            let mut comment_db = if !should_invalidate_cache && comment_cache_path.exists() {
                CommentsDatabase::load(&std::fs::read(&comment_cache_path)?).map_err(|e| {
                    anyhow::anyhow!("Failed to load Nixpkgs comments database: {:?}", e)
                })?
            } else {
